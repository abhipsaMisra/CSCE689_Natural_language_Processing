...
#         print "Error...XML syntax on file: %s, check for mismatched tags..." % (args[1])
      ...
      totalResponseResolutions += responseResolutionCount
      #doing the scoring
      ...
      if responseResolutionCount !=0:
         print "Document Precision %s Score:  %0.2f" % (doc.replace(".key",""), float(correct)/responseResolutionCount)
      else:
         print "Document Precision %s Score:  %0.2f" % (doc.replace(".key", ""), 0.00)
      print "Document Recall %s Score:  %0.2f" % (doc.replace(".key",""), float(correct)/keyResolutionCount)
      if VERBOSE:
         print 

   print "-------------------------------------------------"
   print "Final precision score: %0.4f" % (float(totalCorrect) / totalResponseResolutions)
